...
class UpdateOrchestrationService extends BaseUpdateService
{
    ...
    public function executeUpdate(string $sessionId, array $options = []): array
    {
        ...
        try {
            ...
            $this->sessionService->updateSessionStatus($sessionId, 'in_progress');
            ...
            throw new UpdateException("Update failed: " . $e->getMessage(), 'update_orchestration', 0, $e);
        }
    }
    ...
    private function executePhase1(string $sessionId, array $options): void
    {
        ...
        try {
            ...
            throw new UpdateException("Failed to download release: " . $e->getMessage(), 'download_phase', 0, $e);
        }
        ...
        $validationResult = $this->archiveService->validateArchive($downloadResult['path']);
        if (!$validationResult['is_valid']) {
            $errors = implode('; ', $validationResult['errors'] ?? []);
            throw new UpdateException("Archive validation failed: " . $errors, 'validation_phase');
        }
    ...
    private function executePhase2(string $sessionId, array $options): void
    {
        ...
        try {
            $backupResult = $this->backupService->createFullBackup($options['target_version'], $sessionId);
        } catch (\Exception $e) {
            throw new UpdateException("Backup creation failed: " . $e->getMessage(), 'backup_phase', 0, $e);
        }
    ...
    private function executePhase3(string $sessionId, array $options): void
    {
        ...
        $this->sessionService->updateSessionProgress($sessionId, [
            'current_step' => "Updating {$totalFiles} files",
            'percentage' => 45,
        ]);

        // Convert changes to format expected by FileUpdateService
        $updateFileChanges = [];
        ...
        // Apply file changes
        try {
            $updateResult = $this->fileUpdateService->applyFileChanges($sessionId, $updateFileChanges);
            
            // Check if any files failed to update
            if ($updateResult['failed'] > 0) {
                throw new UpdateException("File update failed: {$updateResult['failed']} files could not be updated", 'file_update_phase');
            }
        } catch (\Exception $e) {
            if (!($e instanceof UpdateException)) {
                $e = new UpdateException("File update failed: " . $e->getMessage(), 'file_update_phase', 0, $e);
            }
            throw $e;
        }
    ...
    private function executePhase4(string $sessionId, array $options): void
    {
        ...
        try {
            // Simple check for pending migrations using Laravel's built-in tools
            $pendingMigrations = \Artisan::call('migrate:status') === 0;
            $migrationAnalysis = [
                'has_migrations' => $pendingMigrations,
                'total_migrations' => $pendingMigrations ? 1 : 0 // Simplified count
            ];
        } catch (\Exception $e) {
            $this->logWarning('Could not check migration status', ['error' => $e->getMessage()]);
            $migrationAnalysis = ['has_migrations' => false, 'total_migrations' => 0];
        }
        ...
        if ($migrationAnalysis['has_migrations']) {
            ...
            // Execute migrations using Laravel's built-in migration system
            try {
    ...
    public function rollbackUpdate(string $sessionId): array
    {
        try {
            ...
            try {
                ...
                throw new UpdateException("Backup restore failed: " . $e->getMessage(), 'rollback_phase', 0, $e);
            }

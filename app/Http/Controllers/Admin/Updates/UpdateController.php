...
class UpdateController extends Controller
{
    ...
    public function checkForUpdates(Request $request): JsonResponse
    {
        try {
            ...
            \Log::error("Exception in initiateUpdate:", ["error" => $e->getMessage(), "trace" => $e->getTraceAsString()]);
            return response()->json([
                'success' => false,
    ...
    public function startUpdate(Request $request, string $version): JsonResponse
    {
        try {
            ...
            // Check if this is a web request vs API request
            \Log::info("Request debugging", [
                "expectsJson" => $request->expectsJson(),
                "ajax" => $request->ajax(),
                "hasWebRequest" => $request->has("web_request"),
                "webRequestValue" => $request->get("web_request"),
            ...
            if ($request->expectsJson() || $request->ajax()) {
                ...
                // Redirect to progress page for web requests
                \Log::info("About to redirect to:", ["route" => "admin.updates.progress-page", "session_id" => $session->session_id]);
                return redirect()->route('admin.updates.progress-page', $session->session_id);
    ...
    {
        \Log::info("=== initiateUpdate method called ===");
        try {
            // Get the latest available version
            $currentVersion = config('app.version', '1.0.0');
            \Log::info("Getting available updates...");
            $availableUpdates = $this->githubService->getAvailableUpdates($currentVersion);
            
            \Log::info("Got available updates, count: " . count($availableUpdates));
            ...
            \Log::info("Available updates check passed, proceeding...");
            // Get the latest version
            $latestUpdate = $availableUpdates[0];
            \Log::info("Latest update found: " . $latestUpdate["tag_name"]);
            ...
            if ($request->expectsJson() || $request->ajax()) {
                ...
                $session = $this->sessionService->createSession([
                    'from_version' => $this->versionService->getCurrentVersion()->version,
                    'to_version' => $latestUpdate['tag_name'],
                    'options' => [
                ...
                // Start the update process asynchronously
                $this->orchestrationService->executeUpdate($session->session_id, [
                    'target_version' => $latestUpdate['tag_name'],
                    'create_backup' => $request->boolean('create_backup', true),
                    'force' => $request->boolean('force', false),
                ]);
        ...
        try {
            ...
            return response()->json([
                ...
                'data' => [
                    ...
                    'recent_logs' => $this->sessionService->getRecentLogs($sessionId, 10),
    ...
    /**
     * Show the update progress page.
     */
    public function showProgressPage(Request $request, string $sessionId)
    {
        \Log::info("showProgressPage called with sessionId: " . $sessionId);
        try {
            $session = $this->sessionService->getSession($sessionId);
            if (!$session) {
                \Log::warning("Session not found: " . $sessionId);
                return redirect()->route('admin.updates.manage')->with('error', 'Update session not found');
            }

            $currentVersion = $this->versionService->getCurrentVersion();
            \Log::info("Showing progress page for session: " . $sessionId);

            return view('admin.updates.progress', [
                'sessionId' => $sessionId,
                'fromVersion' => $session->from_version ?? $currentVersion->version,
                'toVersion' => $session->to_version,
            ]);
        } catch (\Exception $e) {
            \Log::error("Error in showProgressPage: " . $e->getMessage());
            return redirect()->route('admin.updates.manage')->with('error', 'Failed to load progress page: ' . $e->getMessage());
        }
    }
}